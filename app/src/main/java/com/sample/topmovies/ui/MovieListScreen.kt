...
@Composable
fun DP(userProfile: Movie, profilePicSize: Dp) {
    Card(
        shape = CircleShape,
        border = BorderStroke(
            width = 1.dp,
            color = if (userProfile.adult)
        ...
        elevation = 4.dp
            ...
            GlideImage(
            model = Constants.IMAGE_URL_W185 + userProfile.poster_path,
            contentDescription = "",
...
@Composable
fun MovieName(userProfile: Movie, alignment: Alignment.Horizontal) {
    Column(
        modifier = Modifier
            .padding(8.dp),
        ...
        CompositionLocalProvider(
            LocalContentAlpha provides if (userProfile.adult)
                1f else ContentAlpha.medium
        ) {
            Text(text = userProfile.title, style = MaterialTheme.typography.h6)
        }
        CompositionLocalProvider(LocalContentAlpha provides ContentAlpha.medium) {
            Text(
                text = userProfile.release_date,
